  ...
  _setCapabilityOnOff(value, opts, callback){
    ...
    if (opts.relay){
      if (['0', '1'].indexOf(opts.relay) == -1){
        return callback(new Error('invalid opts.relay attribute passed in _setCapabilityOnOff in /lib/device.js'), null);
      }; // if
    } else {
      return callback(new Error('opts doesn\'t have a relay attribute in _setCapabilityOnOff in /lib/device.js'), null);
    }; // if
