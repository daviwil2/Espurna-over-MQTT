...
var settings         = {}; // object to hold the server settings retrieved from the Homey ManagerSettings API; these are not the per-device Advanced settings
...
module.exports = class EspurnaOverMQTTDevice extends Homey.Device {
  ...
  onInit(){
    ...
    .then((advancedSettings) =>{
      ...
      if (writeLogs){ this.log('connecting to MQTT server tcp://'+settings.server+':'+settings.port+' for topic '+rootTopic) };
